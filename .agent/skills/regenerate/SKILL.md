...
- **`geo --write` 뒤에는 `entities/place/` diff를 눈으로 확인하라.** geo는 지명 사전으로 매번 다시 해석하므로 사람이 손으로 교정한 좌표를 원래의 오답으로 되돌린다. 2026-08-13에 `본 곶`이 튀니지 케이프본에서 프랑스 불로뉴쉬르메르로 덮였다(사전이 `Bononia`로 오매칭). 되돌아갔으면 복구한 뒤 다음 단계로 간다.
